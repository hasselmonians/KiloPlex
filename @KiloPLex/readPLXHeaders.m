function [headers, fid, easyread] = readPLXHeaders(filename, fullread)
...
if(~ischar(filename));
    ...
    % what the fuck why would you write over a file?????
...
% so fullread and short read are opposite basically... not sure why this
% code is here...
...
% this goes sequentially through each of these:
...
% time, year, month, day, hour, minute, saecond
...
if(~isscalar(numpointswave))
    ...
    headers.numPointsEachWave = numpointswave;
    ...
    warning('readPLXHeaders:NumPointsWaveMismatch',...
        ...
        'Header Reports %d bits per wave, file contains %d bits per wave. \n'],numpointswave(1),...
        headers.numPointsWave);
    easyread=false;
end
